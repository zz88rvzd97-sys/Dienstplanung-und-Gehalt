...
(function () {
  ...
  const APP_VERSION = '1.27 TV-N Sonderzahlung';
  ...
  function findEffectiveTariffTableKey(monthOrDate) {
    ...
    const keys = tariffTableOptionKeys()
      .filter((key) => {
        const table = getTariffTable(key);
        return Number(table.weeklyHours) === Number(weeklyHours) && table.validFrom && table.validFrom <= target;
      })
  ...
  function getEffectiveTariffForMonth(monthOrDate) {
    ...
    const hourlyRow = table.hourly?.[group];
    if (monthlyRow && hourlyRow && step >= 1 && step <= monthlyRow.length) {
      const hourly = Number(hourlyRow[step - 1]);
      return {
        key,
        table,
        group,
  ...
  function updateTariffPreview() {
    ...
    box.innerHTML = `<strong>TV-N BRB Tabellenwert:</strong> EG ${sel.group}, Stufe ${sel.step} · ${sel.table.weeklyHours} Std./Woche${calc}<br>Monatsentgelt Auswahl: <strong>${euro(sel.monthly)}</strong> · Zuschlagsbasis/Stundenentgelt: <strong>${euro(sel.hourly)}</strong><br><span class="muted">${escapeHtml(sel.table.source)}</span><br><span class="muted">Lohnberechnung nutzt automatisch den je Monat gültigen Stichtag. Aktuell wirksam: ${escapeHtml(effectiveNow.table?.label || effectiveNow.source)} · ${euro(effectiveNow.fixedMonthlyBasePay)} · ${euro(effectiveNow.bonusHourRate)} Zuschlagsbasis.</span>`;
  }
  ...
  function tariffSpecialPaymentsForPaidMonth(month) {
    ...
    if (month === '2026-05') {
      const manualDuplicate = state.statementLines.some(x =>
        ...
        Math.abs(Number(x.amount || 0) - 120) < 0.01
      );
      if (!manualDuplicate) {
        lines.push(item('TARIFF_SPECIAL', 'Sonderzahlung Tarifabschluss Mai 2026', 120, null, null, month, month));
      }
    }
    return lines;
  }
