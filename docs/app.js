...
function renderRotation() {
  ...
  function buildCalendarExportEntries(month) {
    ...
    return Object.values(state.dayEntries)
      ...
      .filter(entry => !isCurrentMonth || entry.date >= today)
      .sort((a,b) => a.date.localeCompare(b.date))
      .flatMap(entry => calendarEntriesForDay(entry));
  }
  ...
  function renderPayroll() {
    ...
    if (exportWrap) {
      ...
      exportWrap.appendChild(dataRow('Quelle', month === currentMonth() ? 'Gespeicherte Tage ab heute bis Monatsende' : 'Gespeicherte Tage dieses Monats'));
      ...
      if (!exportEntries.length) {
        ...
        hint.textContent = 'Noch keine gespeicherten Tage für diesen Monat vorhanden.';
