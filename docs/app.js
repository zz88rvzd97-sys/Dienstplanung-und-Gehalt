...
(function () {
  ...
  const APP_VERSION = '1.14 Cloud Login';
  ...
  function loadState() {
    try {
      ...
      return stripRemovedFeatures(deepMerge(defaultState(), JSON.parse(raw)));
    } catch (e) {
      console.error(e);
      return defaultState();
    }
...
function saveState(options = {}) {
    state = stripRemovedFeatures(state);
  ...
  async function syncCloudAfterLogin(isManualLogin) {
    ...
    try {
      ...
      if (cloudRecord?.data && Object.keys(cloudRecord.data).length) {
        state = stripRemovedFeatures(deepMerge(defaultState(), cloudRecord.data));
        localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
