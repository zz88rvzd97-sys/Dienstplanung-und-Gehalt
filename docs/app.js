...
function saveState() {
  ...
  function buildEarnedItems(month) {
    ...
    for (const date of dates) {
      ...
      if (isWorkingLike) attendanceDays += 1;
  ...
  function getPayrollSourceEntry(date) {
    ...
    if (saved) {
      // FRV zählt erst, wenn ein echter Dienst mit Zeiten oder ein anderer echter Override eingetragen wurde.
      if (saved.actualType === 'frv_open' && !saved.isVacation && !saved.isSick && !saved.isBetriebsversammlung && !(saved.isSplitShift || saved.actualType === 'split_shift') && !(saved.startTime && saved.endTime)) {
        return null;
      }
      return saved;
    }
    const template = getTemplateForDate(date);
    if (!template) return null;
    if (template.templateType !== 'fixed') return null;
    return buildTemplateDayEntry(date);
  }
...
'), location:'' });
  ...
  function exportMonthIcs(monthOverride = '') {
    ...
    a.href = url;
    a.download = `umlauf-kalender-${month}.ics`;
    a.click();
  ...
  function renderPayroll() {
    ...
    if (exportWrap) {
      ...
      exportWrap.appendChild(dataRow('Quelle', 'Direkt aus dem Umlauf dieses Monats'));
