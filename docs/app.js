  ...
  const PAY_TABLES = [
    {
      ...
      label: 'Entgelttabelle 01.04.2026',
      values: {
        15: { 1: 6925, 2: 7121, 3: 7318, 4: 7516, 5: 7714 },
        14: { 1: 6317, 2: 6494, 3: 6673, 4: 6851, 5: 7088 },
        13: { 1: 5766, 2: 5927, 3: 6087, 4: 6248, 5: 6485 },
        12: { 1: 5296, 2: 5429, 3: 5565, 4: 5735, 5: 5966 },
        11: { 1: 4875, 2: 4995, 3: 5115, 4: 5235, 5: 5403 },
        10: { 1: 4490, 2: 4599, 3: 4706, 4: 4813, 5: 4924 },
        9: { 1: 4142, 2: 4238, 3: 4334, 4: 4433, 5: 4529 },
        8: { 1: 3849, 2: 3936, 3: 4023, 4: 4110, 5: 4198 },
        7: { 1: 3593, 2: 3671, 3: 3748, 4: 3827, 5: 3904 },
        6: { 1: 3331, 2: 3401, 3: 3470, 4: 3539, 5: 3608 },
        5: { 1: 3119, 2: 3181, 3: 3243, 4: 3304, 5: 3391 },
        4: { 1: 2988, 2: 3040, 3: 3093, 4: 3151, 5: 3221 },
        3: { 1: 2923, 2: 2972, 3: 3024, 4: 3074, 5: 3126 },
        2: { 1: 2765, 2: 2809, 3: 2855, 4: 2900, 5: 2945 },
        1: { 2: 2600 }
      }
    },
    {
      effectiveFrom: '2027-04-01',
      label: 'Entgelttabelle 01.04.2027',
      values: {
        15: { 1: 7098, 2: 7299, 3: 7501, 4: 7704, 5: 7907 },
        14: { 1: 6475, 2: 6656, 3: 6840, 4: 7022, 5: 7265 },
        13: { 1: 5910, 2: 6075, 3: 6239, 4: 6404, 5: 6647 },
        12: { 1: 5428, 2: 5565, 3: 5704, 4: 5878, 5: 6115 },
        11: { 1: 4997, 2: 5120, 3: 5243, 4: 5366, 5: 5538 },
        10: { 1: 4602, 2: 4714, 3: 4824, 4: 4933, 5: 5047 },
        9: { 1: 4246, 2: 4344, 3: 4442, 4: 4544, 5: 4642 },
        8: { 1: 3945, 2: 4034, 3: 4124, 4: 4213, 5: 4303 },
        7: { 1: 3683, 2: 3763, 3: 3842, 4: 3923, 5: 4002 },
        6: { 1: 3414, 2: 3486, 3: 3557, 4: 3627, 5: 3698 },
        5: { 1: 3197, 2: 3261, 3: 3324, 4: 3387, 5: 3476 },
        4: { 1: 3063, 2: 3116, 3: 3170, 4: 3230, 5: 3302 },
        3: { 1: 2996, 2: 3046, 3: 3100, 4: 3151, 5: 3204 },
        2: { 1: 2834, 2: 2879, 3: 2926, 4: 2973, 5: 3019 },
        1: { 2: 2665 }
      }
    }
  ];
  ...
  function defaultState() {
    return {
      settings: {
        ...
        entgeltgruppe: 'EG 5',
        stufe: 1,
        payGradeEffectiveFrom: '2026-04-01',
        payGradeHistory: [{ effectiveFrom: '2026-04-01', entgeltgruppe: 'EG 5', stufe: 1 }],
...
function saveState() {
  ...
  function calculateEligibleAverageAmountForEntry(entry) {
    const rate = getHourlyRateForMonth(dateToMonth(entry.date)).amount;
    let amount = 0;
  ...
  function buildEarnedItems(month) {
    const items = [];
    const rateInfo = getHourlyRateForMonth(month);
    const rate = rateInfo.amount;
    ...
    for (const date of dates) {
      ...
      if (entry.isBetriebsversammlung && paidHours > 0) items.push(item('BETRIEBSVERSAMMLUNG_HOURS','Std. Betriebsversammlung', paidHours * getHourlyRateForMonth(month).amount, paidHours, null, month, nextMonth(month)));
  ...
  function parseEntgeltGroup(value) {
    const match = String(value || '').match(/\d+/);
    return match ? Number(match[0]) : null;
  }

  function normalizeEntgeltGroup(value) {
    const n = parseEntgeltGroup(value);
    return n ? `EG ${n}` : String(value || '').trim();
  }

  function getActivePayGrade(month) {
    const monthDate = `${month}-01`;
    const history = Array.isArray(state.settings.payGradeHistory) ? state.settings.payGradeHistory : [];
    const valid = history
      .filter(x => x && x.effectiveFrom && x.effectiveFrom <= monthDate)
      .sort((a,b) => String(a.effectiveFrom).localeCompare(String(b.effectiveFrom)));
    if (valid.length) {
      const last = valid[valid.length - 1];
      return {
        entgeltgruppe: normalizeEntgeltGroup(last.entgeltgruppe || state.settings.entgeltgruppe),
        stufe: Number(last.stufe || state.settings.stufe || 1),
        effectiveFrom: last.effectiveFrom
      };
    }
    return {
      entgeltgruppe: normalizeEntgeltGroup(state.settings.entgeltgruppe),
      stufe: Number(state.settings.stufe || 1),
      effectiveFrom: ''
    };
  }

  function getPayTableForMonth(month) {
    const monthDate = `${month}-01`;
    const valid = PAY_TABLES
      .filter(x => x.effectiveFrom <= monthDate)
      .sort((a,b) => String(a.effectiveFrom).localeCompare(String(b.effectiveFrom)));
    return valid.length ? valid[valid.length - 1] : null;
  }

  function getBasePayForMonth(month) {
    const grade = getActivePayGrade(month);
    const table = getPayTableForMonth(month);
    const groupNumber = parseEntgeltGroup(grade.entgeltgruppe);
    const step = Number(grade.stufe || 1);
    const fallback = Number(state.settings.fixedMonthlyBasePay || 3022);

    if (table && groupNumber && table.values[groupNumber] && table.values[groupNumber][step] != null) {
      return {
        amount: Number(table.values[groupNumber][step]),
        entgeltgruppe: `EG ${groupNumber}`,
        stufe: step,
        tableLabel: table.label,
        tableEffectiveFrom: table.effectiveFrom,
        gradeEffectiveFrom: grade.effectiveFrom,
        source: `${table.label} · EG ${groupNumber} Stufe ${step}`
      };
    }

    return {
      amount: fallback,
      entgeltgruppe: grade.entgeltgruppe || state.settings.entgeltgruppe || '—',
      stufe: step,
      tableLabel: 'manueller Fallback',
      tableEffectiveFrom: '',
      gradeEffectiveFrom: grade.effectiveFrom,
      source: `Manueller Monatslohn/Fallback · ${grade.entgeltgruppe || ''} Stufe ${step}`.trim()
    };
  }


  function getRegularWeeklyHoursForMonth(month) {
    // Laut neuer Tabelle bleibt die Monatsentgelt-Tabelle 2027 bestehen; ab 01.01.2028 gilt 38,5 h/Woche.
    return month >= '2028-01' ? 38.5 : 39;
  }

  function getHourlyRateForMonth(month) {
    const basePayInfo = getBasePayForMonth(month);
    const weeklyHours = getRegularWeeklyHoursForMonth(month);
    const divisor = 4.348 * weeklyHours;
    const fallback = Number(state.settings.bonusHourRate || 18.18);

    if (basePayInfo && basePayInfo.tableEffectiveFrom && Number(basePayInfo.amount || 0) > 0 && divisor > 0) {
      return {
        amount: round2(Number(basePayInfo.amount) / divisor),
        weeklyHours,
        divisor: round2(divisor),
        source: `${basePayInfo.entgeltgruppe} Stufe ${basePayInfo.stufe} · ${basePayInfo.tableLabel} · ${weeklyHours} h/Woche`
      };
    }
    ...
    return {
      ...
      weeklyHours,
      divisor: round2(divisor),
      source: 'Manueller Zuschlagsbasis-Fallback'
    };
  }

  function upsertPayGradeHistory(effectiveFrom, entgeltgruppe, stufe) {
    const date = effectiveFrom || currentDate();
    const list = Array.isArray(state.settings.payGradeHistory) ? [...state.settings.payGradeHistory] : [];
    const normalized = { effectiveFrom: date, entgeltgruppe: normalizeEntgeltGroup(entgeltgruppe), stufe: Number(stufe || 1) };
    const idx = list.findIndex(x => x.effectiveFrom === date);
    if (idx >= 0) list[idx] = normalized;
    else list.push(normalized);
    state.settings.payGradeHistory = list.sort((a,b) => String(a.effectiveFrom).localeCompare(String(b.effectiveFrom)));
  }



  function automaticSpecialPayments(month) {
    const items = [];
    if (month === '2026-05') {
      items.push(item('TARIFF_SPECIAL', 'Sonderzahlung Tarifvertrag Mai 2026', 120, null, null, '2026-05', '2026-05'));
    }
    return items;
  }
  ...
  function calculatePayroll(month) {
    ...
    const paidItemsRaw = [
      item('BASE_PAY', `Grundvergütung (${basePayInfo.entgeltgruppe} Stufe ${basePayInfo.stufe})`, basePayInfo.amount, null, null, month, month)
    ];
    paidItemsRaw.push(...automaticSpecialPayments(month));
    ...
    return { month, earnedItems, paidItems, gross, paidGrossTotal, paidAllowancesTotal, basePayInfo, kvbbgUmlage, kvbbgZusatz, estimatedDeductions: estimated, deductions, totalDeductions, deductionMode, payoutPreview, statement };
  }
...
function renderRotation() {
  ...
  function renderPayroll() {
    ...
    document.getElementById('payrollBase').textContent = euro(payroll.basePayInfo.amount);
    ...
    notes.appendChild(dataRow('Grundvergütung', payroll.basePayInfo.source));
    const hourlyInfo = getHourlyRateForMonth(month);
    notes.appendChild(dataRow('Zuschlagsbasis', `${euro(hourlyInfo.amount)} pro Std. · ${hourlyInfo.source}`));
    if (payroll.basePayInfo.gradeEffectiveFrom) notes.appendChild(dataRow('EG/Stufe gültig seit', formatDateLong(payroll.basePayInfo.gradeEffectiveFrom)));
    ...
    if (month === '2026-05') notes.appendChild(dataRow('Automatische Sonderzahlung', '120,00 € Tarif-Sonderzahlung im Mai 2026'));
  ...
  function renderSettings() {
    ...
    setVal('setPayGradeValidFrom', state.settings.payGradeEffectiveFrom || '2026-04-01');
...
function bindEvents() {
    ...
    if (quickAbsenceForm) quickAbsenceForm.addEventListener('submit', saveQuickAbsence);
  ...
  function buildDayEntryFromCurrentForm(baseDate, targetDate = baseDate, options = {}) {
    ...
    const entry = {
      ...
      rangeTo: targetDate,
      parts: [...document.querySelectorAll('#splitPartsContainer .split-part')].map((row) => ({
        startTime: row.querySelector('.split-start')?.value || '',
        endTime: row.querySelector('.split-end')?.value || '',
        breakMinutes: Number(row.querySelector('.split-break')?.value || 0)
      }))
    };
  ...
  function copyCurrentDayToRange() {
    ...
    dates.forEach((targetDate) => {
      if (!shouldApplyToDate(targetDate, onlyWorkdays)) return;
  ...
  function saveSettings(e) {
    ...
    state.settings.entgeltgruppe = normalizeEntgeltGroup(document.getElementById('setGroup').value);
    state.settings.stufe = Number(document.getElementById('setStep').value || 1);
    state.settings.payGradeEffectiveFrom = document.getElementById('setPayGradeValidFrom')?.value || '2026-04-01';
    upsertPayGradeHistory(state.settings.payGradeEffectiveFrom, state.settings.entgeltgruppe, state.settings.stufe);
