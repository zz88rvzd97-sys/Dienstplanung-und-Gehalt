...
function saveState(options = {}) {
    ...
    if (cloudSession && cloudProfile?.active) {
      setSyncStatus('Lokal gespeichert · Cloud wartet · ' + time, 'warning');
    } else {
      setSyncStatus('Lokal gespeichert · ' + time);
    }
  ...
  async function initCloud() {
    try {
      setSyncStatus('Cloud-Verbindung wird geprüft …');
      ...
      console.error(error);
      renderCloudAuth();
      setSyncStatus('Cloud nicht verbunden · lokal aktiv', 'warning');
    }
  }
  ...
  function renderCloudAuth() {
    ...
    if (!cloudSession || !cloudProfile) {
      panel.innerHTML = `
        <form id="cloudLoginForm" class="cloud-auth-form">
          <input id="cloudPersonalnummer" type="text" inputmode="numeric" autocomplete="username" placeholder="Personalnummer" />
    ...
    const roleLabel = cloudProfile.role === 'admin' ? 'Admin' : 'Benutzer';
    const name = cloudProfile.display_name || cloudProfile.personalnummer || 'Benutzer';
    panel.innerHTML = `
      <div class="cloud-user-box">
        <span class="badge ok">Cloud: ${escapeHtml(name)} · ${escapeHtml(roleLabel)}</span>
  ...
  async function handleCloudLogin(e) {
    e.preventDefault();
    const personalnummer = document.getElementById('cloudPersonalnummer')?.value || '';
  ...
  async function syncCloudAfterLogin(isManualLogin) {
    ...
    try {
      ...
      if (cloudRecord?.data && Object.keys(cloudRecord.data).length) {
        state = deepMerge(defaultState(), cloudRecord.data);
        localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
        renderAll();
      ...
      console.error(error);
      setSyncStatus('Cloud-Sync fehlgeschlagen · lokal aktiv', 'danger');
    } finally {
...
function bindEvents() {
    ...
    document.addEventListener('submit', (e) => {
      if (e.target?.id === 'cloudLoginForm') handleCloudLogin(e);
    });
    document.addEventListener('click', (e) => {
