...
(function () {
  ...
  function buildEarnedItems(month) {
    ...
    const dates = Object.keys(state.dayEntries).filter(d => d.startsWith(month)).sort();
    let attendanceDays = 0;
    let fahrdienstDays = 0;
    for (const date of dates) {
      const entry = state.dayEntries[date];
      const paidMinutes = calculatePaidMinutes(entry);
      const paidHours = paidMinutes / 60;
      const isWorkingLike = paidMinutes > 0 || entry.isVacation || entry.isSick;
      if (isWorkingLike && (entry.isFinal || entry.status === 'final' || entry.status === 'assigned' || entry.status === 'open')) attendanceDays += 1;
      if (entry.isFahrdienst && isWorkingLike) fahrdienstDays += 1;
      ...
      if (entry.isFahrdienst && isWorkingLike) items.push(item('FAHRDIENST','Fahrdienstzulage', 5, 1, null, month, nextMonth(month), null, 1));
      if (entry.isSplitShift || entry.actualType === 'split_shift') items.push(item('SPLIT_SHIFT','Geteilter Dienst', 7, null, null, month, nextMonth(month)));
      if (entry.isVacation) {
        const factor = Number(entry.factorValue || 0);
        if (factor > 0) items.push(item('VACATION_BONUS','Urlaubszuschlag', Number(entry.vacationDays || 1) * factor * 7.8, null, null, month, nextMonth(month), factor, Number(entry.vacationDays || 1)));
      }
      if (entry.isSick) {
        const factor = Number(entry.factorValue || 0);
        if (factor > 0) items.push(item('LFZ_BONUS','LFZ-Zuschlag', Number(entry.sickDays || 1) * factor * 7.8, null, null, month, nextMonth(month), factor, Number(entry.sickDays || 1)));
      }
  ...
  function calculatePayroll(month) {
    const earnedItems = buildEarnedItems(month);
    ...
    paidItems.push(...buildEarnedItems(previous).filter(x => x.paidMonth === month));
    paidItems.push(...manualLinesForPaidMonth(month));
    ...
    return { month, earnedItems, paidItems, gross, kvbbgUmlage, kvbbgZusatz, estimatedDeductions: estimated, deductions, totalDeductions, deductionMode, payoutPreview, statement };
  }
...
function renderRotation() {
  ...
  function buildPlannedEntryForDate(date) {
    const template = getTemplateForDate(date);
    const type = template.templateType || 'fixed';
    return {
      date,
      plannedType: type,
      status: type === 'frv' ? 'open' : 'planned',
      actualType: type === 'frv' ? 'frv_open' : type,
      serviceNumber: template.serviceNumber || '',
      startTime: template.defaultStartTime || '',
      endTime: template.defaultEndTime || '',
      breakMinutes: Number(template.defaultBreakMinutes || 0),
  ...
  function buildCalendarExportEntries(month) {
    ...
    const startDate = isCurrentMonth ? today : `${month}-01`;
    const monthDate = new Date(`${month}-01T00:00:00`);
    const endDateObj = new Date(monthDate.getFullYear(), monthDate.getMonth() + 1, 0);
    const endDate = endDateObj.toISOString().slice(0,10);
    const entries = [];
    for (let date = startDate; date <= endDate; date = addDaysIso(date, 1)) {
      const saved = state.dayEntries[date];
      const entry = saved ? { ...buildPlannedEntryForDate(date), ...saved } : buildPlannedEntryForDate(date);
      entries.push(...calendarEntriesForDay(entry));
    }
  ...
  function calendarEntriesForDay(entry) {
    ...
    if (entry.isVacation || entry.actualType === 'vacation') {
      return [{ type:'allDay', date: entry.date, title:'Urlaub', description: buildCalendarDescription(entry), location: state.settings.depotAddress || '' }];
    }
    if (entry.isSick || entry.actualType === 'sick') {
      return [{ type:'allDay', date: entry.date, title:'Krank', description: buildCalendarDescription(entry), location: state.settings.depotAddress || '' }];
    }
    if (entry.actualType === 'frv_open') {
  ...
  function buildCalendarTitle(entry, suffix='') {
    ...
    else if (entry.actualType === 'frv_assigned') base = 'Arbeit FRV-Dienst';
  ...
  function buildMonthIcs(month) {
    ...
    entries.forEach((entry, idx) => {
      ...
      lines.push(`UID:${icsEscape(month)}-${idx}@dienstplanung-gehalt`);
      ...
      if (entry.type === 'allDay') {
        ...
        lines.push(`DTSTART:${toIcsDateTime(entry.date, entry.startTime)}`);
        lines.push(`DTEND:${toIcsDateTime(entry.date, entry.endTime)}`);
  ...
  function exportMonthIcs() {
    ...
    if (!result.entries.length) {
      alert(month === currentMonth() ? 'Ab heute bis Monatsende sind noch keine gespeicherten Kalendereinträge vorhanden.' : 'Für diesen Monat sind noch keine gespeicherten Kalendereinträge vorhanden.');
      return;
    }
  ...
  function renderPayroll() {
    ...
    if (exportWrap) {
      ...
      exportWrap.appendChild(dataRow('Quelle', month === currentMonth() ? 'Umlauf und gespeicherte Tage ab heute bis Monatsende' : 'Umlauf und gespeicherte Tage dieses Monats'));
      if (month === currentMonth()) exportWrap.appendChild(dataRow('Start', formatDateLong(currentDate())));
      if (!exportEntries.length) {
        const hint = document.createElement('div');
        hint.className = 'badge warning';
        hint.textContent = 'Für diesen Zeitraum konnten keine exportierbaren Tage aus Umlauf oder Tagesdaten gefunden werden.';
  ...
  function changeRotationWeek(step) {
    const select = document.getElementById('rotationWeekSelect');
    if (!select) return;
  ...
  function bindEvents() {
    ...
    document.getElementById('prevRotationWeekBtn').addEventListener('click', () => changeRotationWeek(-1));
    document.getElementById('nextRotationWeekBtn').addEventListener('click', () => changeRotationWeek(1));
  ...
  function saveSettings(e) {
    ...
    state.settings.estimatedTaxPercent = Number(document.getElementById('setTaxPercent').value || 0);
    state.settings.estimatedHealthPercent = Number(document.getElementById('setHealthPercent').value || 0);
