...
(function () {
  ...
  const APP_VERSION = '1.17 Umlaufarten & Split-Komfort';
  ...
  function normalizeStateObject(value) {
    const normalized = stripRemovedFeatures(value && typeof value === 'object' ? value : defaultState());
    normalized.settings = normalized.settings && typeof normalized.settings === 'object' ? normalized.settings : {};
    const legacyWeeks = sanitizeRotationWeeks(normalized.rotationWeeks);
  ...
  function loadState() {
    try {
      ...
      return normalizeStateObject(stripRemovedFeatures(deepMerge(defaultState(), JSON.parse(raw))));
    } catch (e) {
      console.error(e);
  ...
  async function syncCloudAfterLogin(isManualLogin) {
    ...
    try {
      ...
      if (cloudRecord?.data && Object.keys(cloudRecord.data).length) {
        state = normalizeStateObject(stripRemovedFeatures(deepMerge(defaultState(), cloudRecord.data)));
        localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
...
function renderRotation() {
  ...
  function renderDayForm() {
    ...
    setSplitQuickValues(entry.parts || []);
    syncDayPaidPreview();
    renderDaysCalendar();
  ...
  function renderDriverDayInfo(dateStr) {
    ...
    const rows = [
      ...
      ['Zeit', entry.startTime && entry.endTime ? `${entry.startTime}–${entry.endTime}` : '—'],
      ['Pause', entry.breakMinutes != null ? `${entry.breakMinutes} Min` : '—'],
  ...
  function setSplitQuickValues(parts = []) {
    const p1 = parts[0] || {};
    const p2 = parts[1] || {};
    const values = {
      splitQuickStart1: p1.startTime || '',
      splitQuickEnd1: p1.endTime || '',
      splitQuickBreak1: String(Number(p1.breakMinutes || 0)),
      splitQuickStart2: p2.startTime || '',
      splitQuickEnd2: p2.endTime || '',
      splitQuickBreak2: String(Number(p2.breakMinutes || 0))
    };
  ...
  function readSplitQuickParts() {
    const parts = [];
    [1, 2].forEach((n) => {
      const start = document.getElementById(`splitQuickStart${n}`)?.value || '';
      const end = document.getElementById(`splitQuickEnd${n}`)?.value || '';
      const brk = Number(document.getElementById(`splitQuickBreak${n}`)?.value || 0);
      if (start || end) parts.push({ startTime: start, endTime: end, breakMinutes: brk });
    });
    return parts;
  }

  function applySplitQuickParts() {
    if (!canEditDays()) return showRoleDenied('Geteilten Dienst erfassen');
    const date = document.getElementById('dayDate')?.value || currentDate();
    const parts = readSplitQuickParts().filter(p => p.startTime && p.endTime);
    if (!parts.length) {
      alert('Bitte mindestens einen vollständigen Arbeitszeitraum mit Beginn und Ende eintragen.');
      return;
    }
    ...
    document.getElementById('actualType').value = 'split_shift';
    document.getElementById('flagSplit').checked = true;
    saveState();
    renderDayForm();
  }

  function clearSplitQuickParts() {
    if (!canEditDays()) return showRoleDenied('Geteilten Dienst leeren');
    ['splitQuickStart1','splitQuickEnd1','splitQuickStart2','splitQuickEnd2'].forEach(id => {
      const el = document.getElementById(id); if (el) el.value = '';
    });
    ['splitQuickBreak1','splitQuickBreak2'].forEach(id => {
...
function bindEvents() {
    ...
    document.getElementById('applySplitQuickBtn')?.addEventListener('click', applySplitQuickParts);
    document.getElementById('clearSplitQuickBtn')?.addEventListener('click', clearSplitQuickParts);
  ...
  function addSplitPart() {
    if (!canEditDays()) return showRoleDenied('Dienstteile bearbeiten');
    const date = document.getElementById('dayDate').value;
    const entry = ensureEntry(date);
  ...
  function importBackup(e) {
    ...
    reader.onload = () => {
      try { state = stripRemovedFeatures(deepMerge(defaultState(), JSON.parse(String(reader.result)))); saveState(); renderAll(); alert('Backup importiert.'); }
