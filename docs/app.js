...
function renderRotation() {
  ...
  function monthDateRange(month) {
    ...
    let startDay = 1;
    if (month === today.slice(0,7)) startDay = Number(today.slice(8,10));
    const dates = [];
  ...
  function materializeTemplateEntry(date) {
    const template = getTemplateForDate(date);
    if (!template) return null;
    const entry = {
      date,
    ...
    // Den Monatsplan aus dem Umlauf fest materialisieren, damit der Export stabil bleibt.
    state.dayEntries[date] = { ...(state.dayEntries[date] || {}), ...entry };
    return state.dayEntries[date];
  }

  function buildCalendarExportEntries(month) {
    return monthDateRange(month)
      .map((date) => materializeTemplateEntry(date))
      .filter(Boolean)
  ...
  function calendarEntriesForDay(entry) {
    ...
    if (entry.actualType === 'fixed' || entry.plannedType === 'fixed') {
      return [{
        type:'allDay',
        date: entry.date,
        title: buildCalendarTitle(entry),
  ...
  function exportMonthIcs(monthOverride = '') {
    ...
    if (!result.entries.length) {
      alert('Für diesen Monat konnten keine Kalendereinträge aus Umlauf oder Tagen erzeugt werden.');
      return;
    }
  ...
  function renderPayroll() {
    ...
    if (exportWrap) {
      ...
      exportWrap.appendChild(dataRow('Quelle', 'Festgeschriebener Monatsplan aus dem Umlauf'));
